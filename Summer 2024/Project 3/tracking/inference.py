...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # Variables for Pacman and Ghosts
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]

    # Define edges according to the diagram (Pacman and Ghosts to Observations)
    edges = [(PAC, OBS0), (PAC, OBS1), (GHOST0, OBS0), (GHOST1, OBS1)]

    # Define the domain of Pacman and Ghosts as all possible positions on the grid
    possible_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ...
    # Observations are noisy Manhattan distances (non-negative) with noise range of ±MAX_NOISE
    # The max possible Manhattan distance is the grid size + the MAX_NOISE
    max_manhattan_distance = X_RANGE + Y_RANGE - 2  # Max Manhattan distance on grid
    possible_observations = list(range(max_manhattan_distance + MAX_NOISE + 1))  # Observations must be non-negative
    variableDomainsDict[OBS0] = possible_observations
    variableDomainsDict[OBS1] = possible_observations
    "*** END YOUR CODE HERE ***"
