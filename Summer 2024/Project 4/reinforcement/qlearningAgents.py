...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.q_values[(state, action)] = (1 - self.alpha) * current_q_value + self.alpha * (reward + self.discount * future_q_value)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        # Compute the difference (same as in normal Q-learning)
        difference = (reward + self.discount * max_next_q_value) - current_q_value
        
        # Get the feature vector for the current (state, action)
        features = self.featExtractor.getFeatures(state, action)
