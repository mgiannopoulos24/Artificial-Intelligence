...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, value in features.items():
            q_value += self.weights[feature] * value  # w * f(s,a)
        return q_value
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Difference = (r + gamma * max(Q(nextState, a')) - Q(state, action))
        correction = (reward + self.discount * self.computeValueFromQValues(nextState)) - self.getQValue(state, action)
        
        features = self.featExtractor.getFeatures(state, action)
        # Update each weight
        for feature, value in features.items():
            self.weights[feature] += self.alpha * correction * value  # wi <- wi + alpha * diff * fi(s,a)
