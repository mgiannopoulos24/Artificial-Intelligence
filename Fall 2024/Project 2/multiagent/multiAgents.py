...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def maxValue(self, gameState, agentIndex, currentDepth):
        ...
        if currentDepth == 0:
            return bestAction  # Return the best action at the root
        else:
            return maxScore  # Return max score at non-root levels
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    # Get scared times for ghosts
    ghostScaredTimes = [ghostState.scaredTimer for ghostState in ghostStates]

    # Initialize evaluation score with the current score
    evaluation = score
    ...
    if food:
        ...
        evaluation += 1.0 / (minFoodDistance + 1)  # Add inverse of distance to the nearest food
    ...
    for ghostState, scaredTime in zip(ghostStates, ghostScaredTimes):
        ...
        if scaredTime > 0:
            ...
            evaluation += 200 / (ghostDistance + 1)  # Encourage chasing scared ghosts
        else:
            # Otherwise, avoid the ghosts
            if ghostDistance <= 2:
                evaluation -= 100  # Big penalty if too close to an active ghost
    ...
    if capsules:
        ...
        evaluation += 10 / (minCapsuleDistance + 1)  # Add inverse of distance to capsules
