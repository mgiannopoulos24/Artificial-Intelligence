...
def check_perceptron(tracker):
    ...
    # Check that forward returns a Tensor, and that the score in the node is correct
    for dimensions in range(1, 10):
        p = models.PerceptronModel(dimensions)
        ...
        assert np.isclose(calculated_score, expected_score), (
            "The score computed by PerceptronModel.forward() ({:.4f}) does not match the expected score ({:.4f})".format(
...
def check_regression(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        assert grad_x[0] != None, "Node returned from regression_loss() does not depend on the provided input (x), check RegressionModel.forward()"
        assert grad_y[0] != None, "Node returned from regression_loss() does not depend on the provided labels (y), check regression_loss()"
    ...
    assert np.isclose(train_loss, sanity_loss), (
        "loss.regression_loss() returned a loss of {:.4f}, "
...
def check_digit_classification(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        assert grad_x[0] != None, "Node returned from digitclassifier_loss() does not depend on the provided input (x), check DigitClassificationModel.forward()"
        assert grad_y[0] != None, "Node returned from digitclassifier_loss() does not depend on the provided labels (y), digitclassifier_loss()"
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        verify_node(output_node, 'tensor', (batch_size, len(dataset.language_names)), "LanguageIDModel.forward()")

        grad = torch.autograd.grad(torch.sum(output_node), inp_xs, allow_unused=True, retain_graph=True)
        for gradient in grad:
            assert gradient != None, "Output returned from LanguageIDModel.forward() does not depend on all of the provided inputs (xs)"
...
def check_attention(tracker):
    ...
    for block_size in [2,4,16]:
        ...
        expected_output = torch.matmul(att_block.q_layer(input),torch.movedim(att_block.k_layer(input),1,2))/layer_size**0.5
